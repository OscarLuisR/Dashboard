<template>
    <v-container>
        ...
        <v-row>
            <v-col class="mb-4">
                <h1 class="display-2 font-weight-bold mb-3">Editar Pelicula</h1>
            </v-col>
        </v-row>
        ...
        methods: {
            async guardarPelicula() {
                try {
                    ...
                    const _id = this.pelicula._id;
                    let router = this.$router;

                    delete this.pelicula._id;

                    const res = await axios.put(`${url}/${_id}`, this.pelicula, options);
